  ...
  return (
    <header className="bg-white border-b border-slate-200/90 sticky top-0 z-30 px-2.5 sm:px-4 lg:px-8 py-2 sm:py-2.5 shadow-2xs">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-1.5 sm:gap-3">
        ...
        {/* Center: Search Bar with Ctrl+K trigger (visible on desktop >= lg for staff only) */}
        {currentUser.role !== 'cliente' && (
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 justify-end shrink-0">
          ...
          {/* Mobile/Tablet Search Button (< lg, staff only) */}
          {currentUser.role !== 'cliente' && (
            <button
              type="button"
              onClick={() => onOpenGlobalSearch && onOpenGlobalSearch()}
          ...
          {/* Botão de Ativação do Modo Edição Master (staff only) */}
          {currentUser.role !== 'cliente' && (
          ...
          {/* TV Reception / Second Screen Button (staff only) */}
          {currentUser.role !== 'cliente' && (
            <button
              type="button"
              onClick={() => {
          ...
          {/* Status de Conexão e Sincronização (staff only) */}
          {currentUser.role !== 'cliente' && <ConnectionSyncStatusWidget />}

          {/* Quadro de Avisos Notification Bell (staff only) */}
          {currentUser.role !== 'cliente' && (
            <button
              type="button"
              onClick={() => {
          ...
          {/* Instalar App (Android e iOS) - staff only */}
          {currentUser.role !== 'cliente' && onOpenPWAInstall && (
            <button
              type="button"
                ...
                {currentUser.role === 'cliente' ? (
                  <>
                    <div className="px-3 py-2 border-b border-slate-100">
                      ...
                      <p className="text-[11px] text-slate-500 truncate">{currentUser.email || 'Conta de Cliente'}</p>
                      <span className="inline-block mt-1 text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
                        Área do Cliente
                      </span>
                    </div>
