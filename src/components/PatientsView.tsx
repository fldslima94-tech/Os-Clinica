...
export const PatientsView: React.FC<PatientsViewProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/90 shadow-sm">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            Prontuários, fichas de anamnese e histórico clínico dos clientes.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white rounded-xl text-xs sm:text-sm font-semibold transition-all cursor-pointer shadow-xs"
          >
      ...
      {/* Search */}
      <div className="bg-white p-4 rounded-3xl border border-slate-200/90 shadow-sm flex items-center justify-between gap-4">
        ...
        <div className="text-xs text-slate-500 font-medium hidden sm:block">
          <strong>{filteredPacientes.length}</strong> cadastrados
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredPacientes.map((paciente) => {
            ...
            return (
              <div
                key={paciente.id}
                className="bg-white rounded-3xl border border-slate-200/90 shadow-sm hover:shadow-md transition-all p-5 flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap justify-end">
                      ...
                      {totalFotos > 0 && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-medium bg-purple-50 text-purple-700">
                          <Camera className="w-3 h-3" />
                          {totalFotos} fotos
                        </span>
                      )}
                      {totalEvolucoes > 0 && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-medium bg-blue-50 text-blue-700">
                  ...
                  <div className="mt-2 space-y-1.5 text-xs text-slate-600">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-slate-800">{paciente.telefone || 'Sem telefone'}</span>
                    </div>
                    ...
                    {paciente.fototipo && (
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-medium uppercase tracking-wider text-slate-400">Fototipo:</span>
                        <span className="font-medium text-slate-700">{paciente.fototipo}</span>
                      </div>
                    )}

                    {paciente.alergias && (
                      <div className="mt-2 p-2 rounded-lg bg-rose-50 border border-rose-100 text-rose-800 text-[11px] flex items-start gap-1.5">
